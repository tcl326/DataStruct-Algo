...
void Balance::checkMatch(const Symbol& oParen, const Symbol & cParen){
    if (oParen.theLine == cParen.theLine) {
        if (!((oParen.token == '(' && cParen.token == ')') || (oParen.token == '{' && cParen.token == '}') || (oParen.token == '[' && cParen.token == ']'))){
            errors++;
            cout << oParen.token << " does not match with " << cParen.token << endl;
        };
    }
}

int Balance::checkBalance(){
    stack<Symbol> balanceStack;
    Symbol ch,prevCh;
    ch.token = tok.getNextOpenClose();
    ch.theLine = tok.getLineNumber();
    while (ch.token != '\0') {
        if (ch.token == '(' || ch.token == '{' || ch.token == '['){
            balanceStack.push(ch);
        }
        else if (!balanceStack.empty()){
            prevCh = balanceStack.top();
            balanceStack.pop();
            checkMatch(prevCh, ch);
        }
        else{
...
int main(int argc, const char * argv[]) {
    // insert code here...
    fstream programFile("/Users/student/Desktop/DataStruct&Algo/HW7/HW7/Tokenizer.cpp", std::fstream::in | std::fstream::out);
    
    Balance test(programFile);
    cout << test.checkBalance();
